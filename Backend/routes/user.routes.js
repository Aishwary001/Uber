...
const userController = require('../controllers/user.controller');  // Fix file name typo
...
router.post('/login',[
    body('email').isEmail().withMessage('Invalid email'),
    body('password').isLength({ min : 6 }).withMessage("Password must be atleat 6 character")
],
